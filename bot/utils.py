...
def _point_sequence_practice_hint(point_index: int, points_count: int, language: str) -> str:
    ...
    if language == "ru":
        if is_first:
            return (
                "Начните только с этой точки. Найдите её по изображению, попробуйте "
                "зацепиться за неё вниманием. Если ощущение слабое или вообще не получается "
                "почувствовать точку, попробуйте помассировать эту точку, мягко надавите "
                "пальцем на эту область, попробуйте круговые движения по часовой и против "
                "часовой стрелке, понаблюдайте, как приятнее. Когда появилось первичное "
                "ощущение, начните представлять дыхание через точку, визуализируйте, как вы "
                "делаете вдох света через эту точку и выдох, синхронизируйте с реальным "
                "дыханием. Вы должны не только представить образ, но и почувствовать некое "
                "ощущение освобождения этой точки. Хорошим результатом будет, если вы "
                "сможете «зацепиться» за точку вниманием без напряжения и специального "
                "действия: появилось намерение почувствовать точку — и вы мгновенно "
                "почувствовали.\n\n"
                "Продолжайте делать так, пока вниманию не станет легче удерживаться здесь. "
                "Продолжайте чувствовать точку.\n\n"
