...
def practice_note(point_index: int, points_count: int, language: str) -> str:
    ...
    if language == "ru":
        if is_first:
            return (
                "Начните только с этой точки. Найдите её по изображению, попробуйте зацепиться за неё вниманием. "
                "Если ощущение слабое или вообще не получается почувствовать точку, попробуйте помассировать эту точку, "
                "мягко надавите пальцем на эту область, попробуйте круговые движения по часовой и против часовой стрелке, "
                "понаблюдайте, как приятнее. Когда появилось первичное ощущение, начните представлять дыхание через точку, "
                "визуализируйте, как вы делаете вдох света через эту точку и выдох, синхронизируйте с реальным дыханием. "
                "Вы должны не только представить образ, но и почувствовать некое ощущение освобождения этой точки. "
                "Хорошим результатом будет, если вы сможете «зацепиться» за точку вниманием без напряжения и специального "
                "действия: появилось намерение почувствовать точку — и вы мгновенно почувствовали.\n\n"
                "Продолжайте делать так, пока вниманию не станет легче удерживаться здесь. Продолжайте чувствовать точку.\n\n"
                "Добейтесь устойчивого ощущения и можете переходить к следующей точке."
            )
        if is_last:
    ...
    function renderSetupComplete() {{
      const labels = {{
        en: {{
          ...
          skip: '📅 Quiet days:',
          nextPrinciples: "From here the practice is simple: return to one principle during the day and notice where it changes thought, speech, or action. The other principles are not paused; today's card is only the doorway.",
          nextMeridians: 'At the chosen time the bot will bring you back to the current meridian or point. Nothing moves forward automatically: you add the next point only when the sensation is ready enough.',
          nextBoth: 'Your rhythm now has two layers: an ethical focus for the day and a body focus through meridians. Let them support each other: less wasted energy, more attention in the body.',
        ...
        ru: {{
          ...
          skip: '📅 Дни тишины:',
          nextPrinciples: 'Дальше всё просто: возвращайтесь к одному принципу в течение дня и замечайте, как он меняет мысли, речь и поступки. Остальные принципы не выключаются; карточка дня только задаёт вход.',
          nextMeridians: 'В выбранное время бот вернёт вас к текущему меридиану или точке. Ничего не двигается вперёд автоматически: следующую точку стоит добавлять только когда ощущение уже достаточно готово.',
          nextBoth: 'Теперь у ритма два слоя: нравственный фокус дня и телесный фокус через меридианы. Пусть они поддерживают друг друга: меньше слитой энергии, больше внимания в теле.',
        ...
        uz: {{
          ...
          skip: '📅 Sokin kunlar:',
          nextPrinciples: "Endi ish oddiy: kun davomida bitta tamoyilga qayting va u fikr, so'z va harakatni qanday o'zgartirishini kuzating. Boshqa tamoyillar to'xtamaydi; kun kartasi faqat kirish eshigi.",
          nextMeridians: "Tanlangan vaqtda bot sizni joriy meridian yoki nuqtaga qaytaradi. Hech narsa avtomatik oldinga siljimaydi: keyingi nuqtani sezgi yetarlicha tayyor bo'lganda qo'shasiz.",
          nextBoth: "Endi ritm ikki qatlamdan iborat: kunning axloqiy fokusi va meridianlar orqali tana fokusi. Ular bir-birini qo'llasin: kamroq energiya yo'qotish, tanada ko'proq diqqat.",
        ...
        kz: {{
          ...
          skip: '📅 Тыныш күндер:',
          nextPrinciples: 'Енді жұмыс қарапайым: күн ішінде бір қағидаға қайта оралып, оның ойды, сөзді және әрекетті қалай өзгертетінін байқаңыз. Қалған қағидалар тоқтамайды; күн картасы тек кіру есігі.',
          nextMeridians: 'Таңдалған уақытта бот сізді ағымдағы меридианға немесе нүктеге қайтарады. Ештеңе автоматты түрде алға жылжымайды: келесі нүктені сезім жеткілікті дайын болғанда қосасыз.',
          nextBoth: 'Енді ырғақ екі қабаттан тұрады: күннің этикалық фокусы және меридиандар арқылы дене фокусы. Олар бірін-бірі қолдасын: энергия аз шашылсын, денеде зейін көбейсін.',
          hint: '/menu қолданып, тізімдерді ашуға, ырғақты өзгертуге немесе келесі шағын қадамды жалғастыруға болады.',
        }},
      }}[state.language];
